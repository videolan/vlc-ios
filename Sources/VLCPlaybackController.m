...
{
    if (toggleFullScreen) {
        // set the previousAspectRatio to current, unless it already is in full screen, so that later
        // when the fullScreen is un-toggled we switch back to what is current
        _previousAspectRatio = _isInFillToScreen ? _previousAspectRatio : _currentAspectRatio;
        
        // if a request to switch directly from/to full screen is made, either switch back to previous or to full screen
        _currentAspectRatio = _isInFillToScreen ? _previousAspectRatio : VLCAspectRatioFillToScreen;
    } else {
        // if switch through the aspect ratios without directly requesting a switch to full screen, then increment current
        // aspect ratio
        _currentAspectRatio = _currentAspectRatio == VLCAspectRatioSixteenToTen ? VLCAspectRatioDefault : _currentAspectRatio + 1;
    }

    // reset isInFillToScreen if fullScreen was toggled directly but then the aspect ratio
    // was changed and fullScreen wasn't previously reset
