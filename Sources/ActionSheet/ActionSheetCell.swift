...
@objcMembers class ActionSheetCellModel: NSObject {
    /// Title `UILabel`'s text that is displayed.
    var title: String
    /// `UIImage` to be displayed as the cell's icon.
    var iconImage: UIImage?
    /// `UIViewController` that is displayed if the cell is selected.
    ...
    ///     - iconImage: image to be displayed as the cell's icon.
    ///     - viewControllerToPresent: view controller that is displayed on cell selection
    ///     - cellAccessoryType cellType: Rightmost accessory view that the cell should use
    init(
        title: String,
