...
@interface VLCServerListViewController () <
    UITableViewDataSource, UITableViewDelegate, UIDocumentPickerDelegate,
    VLCLocalServerDiscoveryControllerDelegate,
    VLCNetworkLoginViewControllerDelegate, VLCRemoteNetworkDataSourceDelegate,
    VLCFileServerViewDelegate> {
...
- (void)traitCollectionDidChange:(UITraitCollection *)previousTraitCollection {
  ...
  /* the event handler in TabBarCoordinator cannot listen to the system because
   * the movie view controller blocks the event Therefore, we need to check the
   * current theme ourselves */
  if (@available(iOS 13.0, *)) {
    ...
    if ([[NSUserDefaults standardUserDefaults]
            integerForKey:kVLCSettingAppTheme] == kVLCSettingAppThemeSystem) {
...
- (instancetype)initWithMedialibraryService:
...
- (void)loadView {
  ...
  _remoteNetworkDataSourceAndDelegate =
      [VLCRemoteNetworkDataSourceAndDelegate new];
  ...
  _localNetworkTableView =
      [[UITableView alloc] initWithFrame:screenDimensions
  ...
  [_remoteNetworkTableView
               registerClass:[VLCWiFiUploadTableViewCell class]
      forCellReuseIdentifier:[VLCWiFiUploadTableViewCell cellIdentifier]];
  [_remoteNetworkTableView registerClass:[VLCRemoteNetworkCell class]
                  forCellReuseIdentifier:VLCRemoteNetworkCell.cellIdentifier];
  [_remoteNetworkTableView
               registerClass:[VLCExternalMediaProviderCell class]
      forCellReuseIdentifier:VLCExternalMediaProviderCell.cellIdentifier];
  ...
  _localNetworkHeight = [_localNetworkTableView.heightAnchor
      constraintEqualToConstant:_localNetworkTableView.contentSize.height];
  _remoteNetworkHeight = [_remoteNetworkTableView.heightAnchor
      constraintEqualToConstant:_remoteNetworkTableView.contentSize.height];

  [NSLayoutConstraint activateConstraints:@[
    [_remoteNetworkTableView.leftAnchor
        constraintEqualToAnchor:self.view.leftAnchor],
    [_remoteNetworkTableView.rightAnchor
        constraintEqualToAnchor:self.view.rightAnchor],
    [_remoteNetworkTableView.topAnchor
        constraintEqualToAnchor:_scrollView.topAnchor],
    [fileServerView.topAnchor
        constraintEqualToAnchor:_remoteNetworkTableView.bottomAnchor],
    ...
    [_localNetworkTableView.topAnchor
        constraintEqualToAnchor:fileServerView.bottomAnchor],
    [_localNetworkTableView.leftAnchor
        constraintEqualToAnchor:self.view.leftAnchor],
    [_localNetworkTableView.rightAnchor
        constraintEqualToAnchor:self.view.rightAnchor],
    [_localNetworkTableView.bottomAnchor
        constraintEqualToAnchor:_scrollView.bottomAnchor],
    _localNetworkHeight, _remoteNetworkHeight
  ]];
}
...
- (void)setupUI {
  ...
  self.tabBarItem =
      [[UITabBarItem alloc] initWithTitle:NSLocalizedString(@"BROWSE", nil)
                                    image:[UIImage imageNamed:@"Network"]
                            selectedImage:[UIImage imageNamed:@"Network"]];
  self.tabBarItem.accessibilityIdentifier =
      VLCAccessibilityIdentifier.localNetwork;
}
...
- (void)viewDidLoad {
  ...
  _discoveryController = [[VLCLocalServerDiscoveryController alloc]
      initWithServiceBrowserClasses:browserClasses];
...
- (void)miniPlayerIsShown {
  _localNetworkTableView.contentInset =
      UIEdgeInsetsMake(0, 0, VLCAudioMiniPlayer.height, 0);
}
...
- (void)connectToServer {
  VLCNetworkLoginViewController *loginViewController =
      [[VLCNetworkLoginViewController alloc]
          initWithNibName:@"VLCNetworkLoginViewController"
                   bundle:nil];

  loginViewController.loginInformation =
      [[VLCNetworkServerLoginInformation alloc] init];
  ;
...
- (void)tableView:(UITableView *)tableView
      willDisplayCell:(VLCNetworkListCell *)cell
    forRowAtIndexPath:(NSIndexPath *)indexPath {
  ColorPalette *themeColors = PresentationTheme.current.colors;
  cell.titleLabel.textColor = cell.folderTitleLabel.textColor =
      cell.thumbnailView.tintColor = themeColors.cellTextColor;
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  VLCNetworkListCell *cell = (VLCNetworkListCell *)[tableView
      dequeueReusableCellWithIdentifier:CellIdentifier];
  if (cell == nil)
    cell = [VLCNetworkListCell cellWithReuseIdentifier:CellIdentifier];

  id<VLCLocalNetworkService> service =
      [_discoveryController networkServiceForIndexPath:indexPath];
...
- (void)tableView:(UITableView *)tableView
  ...
  if ([service respondsToSelector:@selector(serverBrowser)]) {
    ...
    if (serverBrowser) {
      VLCNetworkServerBrowserViewController *vc =
          [[VLCNetworkServerBrowserViewController alloc]
  ...
  if ([service respondsToSelector:@selector(loginInformation)]) {
    ...
    APLog(@"%s: no login information, class %@", __func__,
          NSStringFromClass([service class]));
  }

  /* UPnP does not support authentication, so skip this step */
  if ([login.protocolIdentifier
          isEqualToString:VLCNetworkServerProtocolIdentifierUPnP]) {
    VLCNetworkServerBrowserVLCMedia *serverBrowser;
    if (login.rootMedia != nil) {
      serverBrowser =
          [[VLCNetworkServerBrowserVLCMedia alloc] initWithMedia:login.rootMedia
                                                         options:login.options];
    } else {
      serverBrowser = [VLCNetworkServerBrowserVLCMedia
          UPnPNetworkServerBrowserWithLogin:login];
    }
    VLCNetworkServerBrowserViewController *vc =
        [[VLCNetworkServerBrowserViewController alloc]
            initWithServerBrowser:serverBrowser
...
- (void)showKeychainLoadError:(NSError *)error
                     forLogin:(VLCNetworkServerLoginInformation *)login {
...
- (void)showDocumentPickerViewController:
    (UIDocumentPickerViewController *)viewControllerToPresent {
...
- (void)scheduleLocalTableViewReload {
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    dispatch_after(
        dispatch_time(
            DISPATCH_TIME_NOW,
            (int64_t)(kVLCLocalNetworkReloadDebounceInterval * NSEC_PER_SEC)),
...
- (void)themeDidChange {
  ...
  if (@available(iOS 13.0, *)) {
    UINavigationBarAppearance *navigationBarAppearance =
        [VLCAppearanceManager navigationbarAppearance];
...
- (void)handleRefresh {
  ...
  _refreshControl.attributedTitle = [[NSAttributedString alloc]
      initWithString:NSLocalizedString(@"LOCAL_SERVER_REFRESH", nil)];
  // set the date and time of refreshing
  NSDateFormatter *formattedDate = [[NSDateFormatter alloc] init];
  [formattedDate setDateFormat:@"MMM d, h:mm a"];
  NSString *lastupdated = [NSString
      stringWithFormat:NSLocalizedString(@"LOCAL_SERVER_LAST_UPDATE", nil),
  ...
  _refreshControl.attributedTitle =
      [[NSAttributedString alloc] initWithString:lastupdated
...
- (void)loginWithLoginViewController:
            (VLCNetworkLoginViewController *)loginViewController
                           loginInfo:(VLCNetworkServerLoginInformation *)
                                         loginInformation {
  ...
  if ([identifier isEqualToString:VLCNetworkServerProtocolIdentifierFTP]) {
    serverBrowser = [VLCNetworkServerBrowserVLCMedia
        FTPNetworkServerBrowserWithLogin:loginInformation];
  } else if ([identifier
                 isEqualToString:VLCNetworkServerProtocolIdentifierPlex]) {
    serverBrowser =
        [[VLCNetworkServerBrowserPlex alloc] initWithLogin:loginInformation];
  } else if ([identifier
                 isEqualToString:VLCNetworkServerProtocolIdentifierSMB]) {
    serverBrowser = [VLCNetworkServerBrowserVLCMedia
        SMBNetworkServerBrowserWithLogin:loginInformation];
  } else if ([identifier
                 isEqualToString:VLCNetworkServerProtocolIdentifierNFS]) {
    serverBrowser = [VLCNetworkServerBrowserVLCMedia
        NFSNetworkServerBrowserWithLogin:loginInformation];
  } else if ([identifier
                 isEqualToString:VLCNetworkServerProtocolIdentifierSFTP]) {
    serverBrowser = [VLCNetworkServerBrowserVLCMedia
        SFTPNetworkServerBrowserWithLogin:loginInformation];
  } else {
...
- (void)showEmptyMediaListAlert {
  UIAlertController *alertController = [UIAlertController
      alertControllerWithTitle:NSLocalizedString(@"EMPTY_MEDIA_LIST", "")
                       message:NSLocalizedString(
                                   @"EMPTY_MEDIA_LIST_DESCRIPTION", "")
                preferredStyle:UIAlertControllerStyleAlert];

  UIAlertAction *dismissAction =
      [UIAlertAction actionWithTitle:NSLocalizedString(@"BUTTON_DISMISS", "")
...
- (void)documentPicker:(UIDocumentPickerViewController *)controller
  ...
  if (urls.count > 0) {
    ...
    if (urls.count == 1 && [[urls[0] pathExtension] isEqualToString:@""]) {
      ...
      NSSortDescriptor *urlSortDescriptor = [NSSortDescriptor
          sortDescriptorWithKey:@"absoluteString"
                      ascending:YES
                       selector:@selector(localizedStandardCompare:)];
      NSArray<NSURL *> *sortedArray =
          [urls sortedArrayUsingDescriptors:@[ urlSortDescriptor ]];
...
- (void)getFolderData:(NSURL *)url mediaList:(VLCMediaList *)list {
  ...
  NSArray<NSURL *> *filesInFolder = [[NSFileManager defaultManager]
